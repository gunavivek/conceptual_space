#!/usr/bin/env python3
"""
Q2.7: Complete Assignment Hierarchy Visualization
Shows the full assignment chain: Chunks → Concepts → Question

Part of the Q2.7 Visualization Series (separated from Q2.5 Assignment Logic)
...
class Q25CompleteAssignmentVisualizer:
    ...
    def load_data(self, doc_id: str):
        ...
        q25_path = os.path.join(
            ...
            'outputs', 'Q2.5_document_aware_assignment.json'
        )

        try:
            with open(q25_path, 'r', encoding='utf-8') as f:
                q25_full = json.load(f)
                # Handle new Q2.5 batch structure with question_results
                if 'question_results' in q25_full and doc_id in q25_full['question_results']:
                    self.q25_data = q25_full['question_results'][doc_id]
                    print(f"[SUCCESS] Q2.5 data loaded for {doc_id}")
                elif doc_id in q25_full:  # Fallback for old structure
