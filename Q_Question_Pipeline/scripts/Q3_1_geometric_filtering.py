...
class Q31_ConstrainedGeometricMatching:
    ...
    def load_question_data(self, question_id: str) -> Dict:
        ...
        if 'question_results' in q25_data:
            ...
            # Extract the data structure expected by Q3.1
            assignment_data = question_data['multi_dimensional_analysis']['document_aware_assignment']

            # Restructure to match Q3.1 expectations
            return {
                'question_id': question_data['question_id'],
                'doc_id': question_data['doc_id'],
                'convex_ball_assignments': assignment_data['convex_ball_assignments'],
                'coordinate_space': question_data.get('coordinate_space', {'coordinates': []}),
                'membership_statistics': assignment_data['membership_statistics']
            }
        else:
            # Legacy format
...
if __name__ == "__main__":
    ...
    try:
        ...
        if os.path.exists(q25_path):
            ...
            # Process all questions from batch format
            if 'question_results' in q25_data:
                question_ids = list(q25_data['question_results'].keys())
                print(f"\nFound {len(question_ids)} questions to process")
            else:
                question_ids = list(q25_data.keys())
                print(f"\nFound {len(question_ids)} questions to process (legacy format)")

            # Process all questions through Q3.1
            processed_count = 0
            for question_id in question_ids:
                try:
                    print(f"\nProcessing question {processed_count + 1}/{len(question_ids)}: {question_id}")
                    q31_output = q31.process_question(question_id)

                    # Save output for this question
                    q31.save_output(q31_output)
                    processed_count += 1

                    # Show brief summary for this question
                    if q31_output['constrained_matches']:
                        print(f"  [OK] Found {len(q31_output['constrained_matches'])} matches")
                    else:
                        print(f"  [WARN] No matches found (reason: {q31_output['constraint_metrics'].get('constraint_satisfied', 'unknown')})")
            ...
            print(f"\n{'='*60}")
            print(f"Q3.1 BATCH PROCESSING COMPLETE")
            print(f"{'='*60}")
            print(f"Successfully processed: {processed_count}/{len(question_ids)} questions")

            # Show summary of first processed question for detailed output
