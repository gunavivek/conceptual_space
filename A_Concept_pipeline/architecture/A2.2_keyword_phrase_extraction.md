...
- **Secondary**: `outputs/A1.2_concept_enriched_documents.json`
- **Contains**: Cleaned documents + BIZBOK concept definitions, matched keywords, and concept enrichment data for intelligent keyword weighting
...
### Stage 1: BIZBOK Concept Intelligence Integration (ENHANCED)
- **Dual Input Loading**: Loads both A2.1 preprocessed documents and A1.2 concept enrichment data  
...
- **Concept-Aware Weighting**: Applies relevance multipliers to TF-IDF scores based on concept enrichment alignment
...
### Core Functions
- **`load_input(preprocessed_path, domain_path)`**: Loads dual inputs from A2.1 and A1.2
  - Primary: `A2.1_preprocessed_documents.json` for cleaned text
  - Secondary: `A1.2_concept_enriched_documents.json` for concept intelligence
- **`extract_keywords_enhanced()`**: Main extraction with BIZBOK enhancement
- **`calculate_tfidf()`**: TF-IDF calculation with concept-aware boosting
- **`apply_domain_boost()`**: Applies domain-specific term multipliers
- **`extract_phrases()`**: Multi-word phrase detection and scoring

### Input File Updates
- Changed from: `A1.2_domain_detection_output.json` (old)
- Changed to: `A1.2_concept_enriched_documents.json` (current)
- Reason: A1.2 was transformed from domain detection to concept enrichment

### Integration with Enhanced Pipeline
- Consumes table-converted text from A1.2 (monetary values properly formatted)
- Processes sentences correctly split by A2.1 (decimal numbers preserved)
- Leverages concept intelligence for domain-aware keyword extraction