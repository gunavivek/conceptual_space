...
- **Contains**: Documents with **table-to-text conversions**, cleaned text, **lemmatized tokens**, **POS tags**, **noun phrases**, **business terms**, extracted sentences/paragraphs, word counts, table detection statistics, and comprehensive preprocessing metrics
...
### Stage 3: Linguistic Analysis and Lemmatization (NEW - ENHANCED)
- **Lemmatization Processing**: Converts word forms to their base lemmas using spaCy NLP
  - Normalizes inflected forms (running → run, companies → company)
  - Creates token mappings for reconstruction
  - Extracts unique lemmas for vocabulary analysis
- **Part-of-Speech Tagging**: Identifies grammatical roles for each token
  - Captures POS tags (NOUN, VERB, ADJ, etc.)
  - Identifies stop words and functional tokens
  - Enables syntactic pattern matching
- **Noun Phrase Extraction**: Identifies multi-word business terms
  - Extracts noun chunks with their lemmatized forms
  - Captures compound nouns and modifier patterns
  - Preserves phrase-level semantic units
- **Business Term Identification**: Extracts domain-relevant terminology
  - Filters noun phrases containing business keywords
  - Identifies compound noun patterns (NN + NN)
  - Creates curated list of potential business concepts

### Stage 4: Semantic Structure Extraction (DECIMAL-PRESERVING)
...
### Lemmatization Integration (NEW)
- **Decision**: Add spaCy-based lemmatization and POS tagging as Stage 3 processing
- **Rationale**: Normalizes word forms to improve keyword extraction quality and reduces redundancy in downstream processing
- **Impact**: Enables A2.2 to work with normalized base forms (company vs companies) and leverage grammatical structure for better business term identification

### Business Term Pre-Extraction (NEW)
- **Decision**: Identify potential business terms during preprocessing using POS patterns and domain keywords
- **Rationale**: Early identification of business concepts provides seed terms for A2.2 ensemble keyword extraction
- **Impact**: Creates foundation for domain-aware keyword extraction and reduces noise in concept formation
