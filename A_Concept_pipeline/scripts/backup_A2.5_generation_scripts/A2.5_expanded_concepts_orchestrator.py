...
def combine_expansion_results(strategy_results):
    ...
    for strategy_name, data in strategy_results.items():
        if data and "results" in data:
            # Handle expansion strategies (A2.5.4, A2.5.5)
            if "expansions" in data["results"]:
                expansions = data["results"]["expansions"]
                for expansion in expansions:
                    concept_id = expansion["original_concept"]["concept_id"]
                    all_concept_ids.add(concept_id)
            # Handle generation strategies (A2.5.1, A2.5.2, A2.5.3)
            elif "generated_concepts" in data["results"]:
                generated = data["results"]["generated_concepts"]
                for concept in generated:
                    seed_id = concept.get("seed_concept_id")
                    if seed_id:
                        all_concept_ids.add(seed_id)
    ...
    for concept_id in all_concept_ids:
        ...
        for strategy_name, data in strategy_results.items():
            ...
            # Handle generation strategies (A2.5.1, A2.5.2, A2.5.3)
                ...
                # For generation strategies, collect all generated concepts for this seed
                generated_for_seed = [
                    concept for concept in data["results"]["generated_concepts"]
                    if concept.get("seed_concept_id") == concept_id
                ]
                if generated_for_seed:
                    # Create a synthetic expansion from generated concepts
                    concept_expansion = {
                        "original_concept": {"concept_id": concept_id},
                        "generated_concepts": generated_for_seed
                    }
            ...
            # Handle generation strategies (A2.5.1, A2.5.2, A2.5.3)
            if "generated_concepts" in concept_expansion:
                for generated_concept in concept_expansion["generated_concepts"]:
                    keywords = generated_concept.get("keywords", generated_concept.get("primary_keywords", []))
                    strategy_terms.update(keywords)
