...
def calculate_term_frequency_stats(concepts):
    ...
    doc_terms = defaultdict(set)  # Terms in each actual document
    document_ids = set()  # Track unique documents

    # Group terms by actual document
    for concept in concepts:
        keywords = concept.get("keywords", concept.get("primary_keywords", []))
        doc_id = concept.get("document_id", "unknown")

        document_ids.add(doc_id)

        # Add all keywords to this document's term set
        for keyword in keywords:
            doc_terms[doc_id].add(keyword)
    ...
    for doc_id, terms in doc_terms.items():
        for term in terms:
            term_doc_freq[term] += 1  # Count documents containing this term
            term_total_freq[term] += 1  # Simple frequency count
    ...
    for term in term_doc_freq:
        ...
        # IDF calculation - use actual document count, not concept count
        idf = math.log(max(total_documents, 1) / max(df, 1))
        ...
        term_stats[term] = {
            ...
            "idf": idf,
            "tf_idf": tf_idf,
            "prevalence": df / max(total_documents, 1)  # What fraction of documents contain this term
        }
...
def find_high_frequency_expansions(concept, term_stats, threshold_percentile=75):
    ...
    """
    concept_keywords = set(concept.get("keywords", concept.get("primary_keywords", [])))
...
def expand_concept_frequency_based(concept, term_stats, doc_terms):
    ...
    all_expansion_terms = set(concept.get("keywords", concept.get("primary_keywords", [])))
    ...
    original_count = len(concept.get("keywords", concept.get("primary_keywords", [])))
