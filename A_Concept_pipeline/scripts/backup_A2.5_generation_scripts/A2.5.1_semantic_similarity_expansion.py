...
def calculate_semantic_similarity(concept1, concept2):
    ...
    kw1 = set(concept1.get("keywords", concept1.get("primary_keywords", [])))
    kw2 = set(concept2.get("keywords", concept2.get("primary_keywords", [])))
...
def generate_semantic_neighbor_concepts(seed_concept, all_concepts, expansion_id_base):
    ...
    new_concepts = []
    seed_keywords = set(seed_concept.get("keywords", seed_concept.get("primary_keywords", [])))
    ...
    if len(high_sim_concepts) >= 2:
        ...
        for sim_data in high_sim_concepts[:3]:  # Top 3 high-similarity
            ...
            clustered_keywords.update(sim_concept.get("keywords", sim_concept.get("primary_keywords", [])))
    ...
    for i, sim_data in enumerate(similar_concepts[:4]):  # Top 4 individual neighbors
        ...
        neighbor_keywords = set(sim_concept.get("keywords", sim_concept.get("primary_keywords", [])))
    ...
    if cross_domain_concepts:
        ...
        for cross_sim in cross_domain_concepts[:2]:  # Top 2 cross-domain
            ...
            bridge_keywords.update(cross_concept.get("keywords", cross_concept.get("primary_keywords", [])))
